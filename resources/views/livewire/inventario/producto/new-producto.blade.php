    ...
    <div class="grid grid-cols-2 gap-3">
        ...
        @if ($productoArray['categoria'] == 'Combo')
            <div class="mb-4">
                ...
                <select id="combo" wire:model="productoArray.combo_id"
                    class="bg-gray-50 border border-gray-300 text-gray-900 mb-6 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option selected value="0">Selecciona un combo</option>
                    @foreach ($listaCombos as $combo)
                        <option value="{{ $combo->id }}">{{ $combo->nombre }} - {{ $combo->costo_total }}Bs. </option>
                    @endforeach
                </select>
